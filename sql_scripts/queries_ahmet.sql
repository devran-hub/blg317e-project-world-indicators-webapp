...
------------------------------------------------------------
-- 4. Population density by country in 2020
------------------------------------------------------------
SELECT
    r.region_name,
    c.country_name,
    ROUND(pop.population_2020 / area.area_2020, 2) AS population_density
FROM Countries c
JOIN Regions r ON c.region_id = r.id

-- Population values (2020)
JOIN (
    SELECT 
        country_id,
        value AS population_2020
    FROM IndicatorData id
    JOIN Indicators i ON id.indicator_id = i.id
    WHERE id.year = 2020
      AND i.indicator_name = 'Population'
) pop ON pop.country_id = c.id

-- Land area values (2020)
JOIN (
    SELECT 
        country_id,
        value AS area_2020
    ...
    JOIN Indicators i ON id.indicator_id = i.id
    WHERE id.year = 2020
      AND i.indicator_name = 'Land area (sq. km)'
) area ON area.country_id = c.id

ORDER BY r.region_name, population_density DESC;


------------------------------------------------------------
-- 5. Countries with decreasing CO₂ emissions (2010–2020)
------------------------------------------------------------
SELECT 
    c.country_name,
    (MAX(idata.value) - MIN(idata.value)) AS decrease_amount
FROM IndicatorData idata
JOIN Indicators i ON idata.indicator_id = i.id
JOIN Countries c ON idata.country_id = c.id
WHERE i.indicator_name = 'CO2 emissions'
  AND idata.year BETWEEN 2010 AND 2020
GROUP BY c.country_name
HAVING MAX(idata.value) > MIN(idata.value)
